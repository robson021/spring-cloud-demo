...
public class CountryServiceRoutes {
    ...
    private Mono<ServerResponse> getCapitalCity(ServerRequest request) {
        return ServerResponse.ok().contentType(APPLICATION_JSON).body(getCountryInfo(request.pathVariable("capitalCity")), CountryDTO.class);
    }
    ...
    private Mono<ServerResponse> getAllCountries(ServerRequest request) {
        return ServerResponse.ok().contentType(APPLICATION_JSON).body(repository.findAll(), CountryEntity.class);
    }
}
